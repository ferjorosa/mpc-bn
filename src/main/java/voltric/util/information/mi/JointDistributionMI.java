...
class JointDistributionMI {
    ...
    public static double computePairwise(List<DiscreteVariable> x, List<DiscreteVariable> y, Function dist){
        ...
        if(x.containsAll(y))
            return JointDistributionEntropy.compute(py);
        if(y.containsAll(x))
            return JointDistributionEntropy.compute(px);
        ...
        double Hxy = JointDistributionEntropy.compute(dist);
        double HcondXY = JointDistributionEntropy.computeConditional(dist, y);
        double HcondYX = JointDistributionEntropy.computeConditional(dist, x);
