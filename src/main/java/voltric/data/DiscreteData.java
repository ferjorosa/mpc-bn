...
public class DiscreteData {
     ...
     *
     * Note: It is a double value for easier computations (i.e. FrequencyCountedEntropy, Mutual Information, etc.). However it can only
    ...
    private List<DiscreteVariable> sortWithOriginalIndex(List<DiscreteVariable> variables){
        ...
        Comparator<SymmetricPair<Integer, DiscreteVariable>> byIndex =
                (pair1, pair2) -> Integer.compare(pair1.getFirst(), pair2.getFirst());

        // Pseudo zip with index
        return this.variables.stream()
                .filter(variables::contains)
                .map(x-> new SymmetricPair<>(this.variables.indexOf(x), x))
                .sorted(byIndex)
                .map(SymmetricPair::getSecond)
