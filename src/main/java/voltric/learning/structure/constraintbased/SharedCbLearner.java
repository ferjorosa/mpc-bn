...
public class SharedCbLearner implements CBLearner {
    ...
    private Map<Map<SymmetricPair<DiscreteVariable,DiscreteVariable>, List<DiscreteVariable>>, Double> ciTestScores;

    public SharedCbLearner(int maxParents,
                           double p_value,
                           CITest ciTest,
                           Map<Map<SymmetricPair<DiscreteVariable,DiscreteVariable>, List<DiscreteVariable>>, Double> concurrentCiTestScores){
    ...
    public UndirectedGraph<DiscreteVariable> learnSkeleton(DiscreteData data){
        ...
        while(currentOrder <= maxParentOrder){
            ...
            for(UndirectedNode<DiscreteVariable> node: graph.getUndirectedNodes()){ // x in X
                ...
                List<SymmetricPair<AbstractNode<DiscreteVariable>, AbstractNode<DiscreteVariable>>> edgesToBeRemoved = new ArrayList<>();
                    ...
                    if (conditionalNeighbors.size() >= currentOrder) {
                        ...
                        while (condNeighCombinations.hasNext() && !conditionalIndecencyFound) {
                            ...
                            if (testValue < p_value) {
                                ...
                                edgesToBeRemoved.add(new SymmetricPair<>(neighbor, node));
                            }
                        }
                    }
                }// end neighbor iteration

                for(SymmetricPair<AbstractNode<DiscreteVariable>, AbstractNode<DiscreteVariable>> undirectedEdge: edgesToBeRemoved)
