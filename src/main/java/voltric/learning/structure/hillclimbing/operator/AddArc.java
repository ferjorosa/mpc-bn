...
public class AddArc implements HcOperator{
    ...
    /** {@inheritDoc} */
    public LearningResult<DiscreteBayesNet> apply(final DiscreteBayesNet seedNet, DiscreteData data, DiscreteParameterLearning parameterLearning){

        // The BN is copied to avoid modifying current object.
        DiscreteBayesNet clonedNet = seedNet.clone();

        DiscreteBeliefNode bestEdgeHead = null;
        DiscreteBeliefNode bestEdgeTail = null;
                ...
                if (!fromNode.equals(toNode)) {
                    ...
                    if ((!edgeBlackList.containsKey(fromNode.getVariable()) || !edgeBlackList.get(fromNode.getVariable()).contains(toNode.getVariable())) &&
                        ...
                        toNode.getParentNodes().size() < this.maxNumberOfParents) {
                            ...
                            try {
                                Edge<Variable> newEdge = clonedNet.addEdge(toNode, fromNode);
                                ...
                                // If an invalid edge is created an exception will be thrown.
        ...
        // If the model has been modified, it needs to be relearned
        if(bestEdgeHead != null && bestEdgeTail != null) {
            resultBN.addEdge(bestEdgeHead, bestEdgeTail);
