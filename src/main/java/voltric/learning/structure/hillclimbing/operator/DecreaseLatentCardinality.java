...
public class DecreaseLatentCardinality implements HcOperator {
    ...
    public LearningResult<DiscreteBayesNet> apply(DiscreteBayesNet seedNet, DiscreteData data, DiscreteParameterLearning parameterLearning) {
        ...
        // If the model has been modified (the cardinality of the LV has been decreased) it needs to be relearned
        if(bestLatentVar != null) {
            resultBN = clonedNet.decreaseCardinality(clonedNet.getLatentVariable(bestLatentVar.getName()), 1);
            resultBN = parameterLearning.learnModel(resultBN, data).getBayesianNetwork();
        }

        return new LearningResult<>(resultBN, bestScore, parameterLearning.getScoreType());
    }
}
