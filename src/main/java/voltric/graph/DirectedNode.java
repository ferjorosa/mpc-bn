...
public class DirectedNode<T> extends AbstractNode<T> {
    ...
    protected void attachInEdge(Edge<T> edge) {
        ...
        parents.put(edge.getTail().content, edge);
    }
    ...
    protected final void attachOutEdge(Edge<T> edge) {
        ...
        children.put(edge.getHead().content, edge);
    }
    ...
    protected void detachInEdge(Edge<T> edge) {
        // Remove the parent from the 'neighbors' collection
        this.detachEdge(edge);
        // removes tail from the collection of parents of this node
        parents.remove(edge.getTail().getContent());
    }
    ...
    protected final void detachOutEdge(Edge<T> edge) {
        // Remove the child from the 'neighbors' collection
        this.detachEdge(edge);
        // removes head from the collection of children of this node
        children.remove(edge.getHead().getContent());
    }
    ...
    public final Set<DirectedNode<T>> getChildren() {
        return children.keySet().stream().map(x-> (DirectedNode<T>) graph.getNode(x)).collect(Collectors.toCollection(LinkedHashSet::new));
    }
    ...
    public final Set<DirectedNode<T>> getParents() {
        return parents.keySet().stream().map(x-> (DirectedNode<T>) graph.getNode(x)).collect(Collectors.toCollection(LinkedHashSet::new));
    }
    ...
    public final boolean hasChild(AbstractNode<T> node) {
        ...
        return children.containsKey(node.getContent());
    }
    ...
    public final boolean hasParent(AbstractNode<T> node) {
        ...
        return parents.containsKey(node.getContent());
    }
