...
public class AssignToClusters {
    ...
    public static List<SymmetricPair<DiscreteDataInstance, List<double[]>>> assignDataCaseToClusters(DiscreteData dataSet, DiscreteBayesNet ltm) {

        List<SymmetricPair<DiscreteDataInstance, List<double[]>>> partitionCpts = new ArrayList<>();
        ...
        for (DiscreteDataInstance dataCase : projectedData.getInstances()) {
            ...
            partitionCpts.add(new SymmetricPair<>(dataCase,latentVarCells));
        }
        return partitionCpts;
    }

    // Solo 1 LV
    public static List<SymmetricPair<DiscreteDataInstance, double[]>> assignDataCaseToCluster(DiscreteData data, DiscreteBayesNet lcm) {
        ...
        List<SymmetricPair<DiscreteDataInstance, double[]>> clusterAssignments = new ArrayList<>();
        Map<DiscreteVariable, Integer> evidence = new HashMap<>();
        ...
        for (DiscreteDataInstance dataCase : projectedData.getInstances()) {
            ...
            clusterAssignments.add(new SymmetricPair<>(dataCase, latentVarFunc.getCells()));
        }
