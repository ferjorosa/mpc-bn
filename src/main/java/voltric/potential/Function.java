 ...
 *
 *
 * {@link #computeIndex(int[])}
 * {@link #computeStates(int, int[])}
 *
 * @author Yi Wang
 *
 */
public class Function implements Cloneable, Potential {
    ...
    private static Uniform rndGenerator = new Uniform(new MersenneTwister(new Date()));
     ...
     *
     * @param variables list of variables to be involved.
     ...
     */
    public final static Function createFunction(DiscreteData dataSet) {
        ...
        //TODO: Esta es una razon por la cual interesa hacer DataSets inmutables, al menos en el aspecto de las variables,
        //TODO: ya que no tiene sentido que se tenga que crear una copia entera d la lista de variables por cada function de la BN
        ...
        // order.
        Function f = Function.createFunction(new ArrayList<>(dataSet.getVariables()));

        //TODO: No me gusta nada l hecho de que tenga que hacer castings de todos los dataCases
        // Seguramente tenga que crear 3 tipos de DataSet, no basta con generalidad para este caso
        // IDilicamente habria que testear la velocidad

        // fills in cells by going through all data cases
        for (DiscreteDataInstance dataCase : dataSet.getInstances()) {
            f._cells[f.computeIndex(dataCase.getNumericValues())] = dataSet.getWeight(dataCase);
        }
     ...
     */
    private final Function createFunction(DiscreteVariable[] variables, double[] cells, int[] magnitudes) {
        Function f = null;
     ...
     * @return an identity function.
     */
    public final static Function createIdentityFunction(DiscreteVariable variable) {
     ...
     */
    public final static Function createUniformDistribution(DiscreteVariable variable) {
        Function f = new Function1D(new DiscreteVariable[] { variable });
     ...
     *            array of magnitudes for variables in new function.
     */
    protected Function(DiscreteVariable[] variables, double[] cells, int[] magnitudes) {
        _variables = variables;
        _cells = cells;
    ...
    ///////////////////////////////////////////////////////////
    // TODO: MEtodos propios de los Potenciales continuos
    ///////////////////////////////////////////////////////////
     ...
     * at the {@state}.
     *
     * @param variable variable of the indicator function
     * @param state state which has value 1, and other unspecified states 0
     */
    @Override
    ...
    public final void computeStates(int index, int[] states) {
        ...
        for (int i = 0; i < dimension; i++) {
            if (index == 0) {
                // states of remaining variables are all zeros (First case is always 0s i.e., P(A=0, B=0,...))
    ...
    public Function combine(DiscreteVariable var, int si, int sj, DiscreteVariable newVar) {
        ...
        if(!var.isValuePermitted(si) || !var.isValuePermitted(sj))
        ...
        ArrayList<DiscreteVariable> variables = new ArrayList<DiscreteVariable>(dimension);
     ...
     * @return
     */
    public Function combine(DiscreteVariable var, int[] statesToMerge, int stateToKeep, DiscreteVariable newVar) {
     ...
     *            is increased by one.
     * @return A new Function through split the state s of DiscreteVariable var.
     */
     ...
     * AbstractBeliefNode. Therefore this implemenation can be not efficient.
     * </p>
     *
     * @param variables
    ...
    public final double[] getCells(List<DiscreteVariable> variables) {
        ...
        Function f = new Function(variables.toArray(new DiscreteVariable[variables
                .size()]));
     ...
     * and <code>CliqueTreePropagation.computeFamilyBelief(AbstractBeliefNode)</code>.
     ...
     * @see voltric.inference.CliqueTreePropagation#computeBelief(Collection)
     * @see voltric.inference.CliqueTreePropagation#computeFamilyBelief(DiscreteVariable)
     */
    ...
    public final double normalize() {
        ...
        if (sum != 0.0) {
            ...
            // uniformly distributes it if normalizing constant equals 0
            Arrays.fill(_cells, 1.0 / getDomainSize());
            // TODO: Note: Arreglado por mi a partir de aqui, daba un error en CliqueTreePropagation.sendMessage al no normalizar bien
            sum = sumUp();
            divide(sum);
        }
    ...
    /**
     * <p>
     * We suppose the argument DiscreteVariable, denoted by Y, is in the function. The
    ...
    public final void setCell(ArrayList<DiscreteVariable> variables, ArrayList<Integer> states, double cell) {
        ...
        for (int i = 0; i < dimension; i++) {
            // state must be valid
            if(!variables.get(i).isValuePermitted(states.get(i)))
     ...
     *
     * TODO: It doesn't sort the cells, so its kinda useless
     *
     * @param variables
     ...
     *            new values of cells.
     */
    public final void setCells(List<DiscreteVariable> variables, List<Double> cells) {
        int dimension = getDimension();
        int domainSize = getDomainSize();
        ...
        // we play a trick here. the argument variables are not necessary
        // sorted, but we builds function of them for indexing.
        Function f = new Function(variables.toArray(new DiscreteVariable[variables.size()]));
     ...
     * TODO: It doesn't sort the cells, so its kinda useless
     */
    public final void setCells(List<DiscreteVariable> variables, double[] cells) {
        int dimension = getDimension();
        int domainSize = getDomainSize();
        ...
        // we play a trick here. the argument variables are not necessary
        // sorted, but we build function of them for indexing.
        Function f = new Function(variables.toArray(new DiscreteVariable[variables.size()]));
    ...
    public Function sumOut(List<DiscreteVariable> variables){

        Function summedOut = this;

        for(DiscreteVariable variable: variables)
            summedOut = summedOut.sumOut(variable);
    ...
    public void multiply(Function function) {
        ...
        int gIndex = 0;
        while (true) {
            if (function._variables[gIndex].equals(_variables[fIndex])) { // TODO: 25-10-2017 -> He cambiado '==' por equals
                gMap[gIndex++] = fIndex++;
                if (gIndex == gDim)
                    break;
            } else {
    ...
    public void divide(Function function) {
        ...
        int gIndex = 0;
        while (true) {
            if (function._variables[gIndex].equals(_variables[fIndex])) { // TODO: 25-10-2017 He cambiado el '==' por equals
                gMap[gIndex++] = fIndex++;
                if (gIndex == gDim)
                    break;
            } else {
    ...
    public Function times(Function function) {
        ...
        ArrayList<DiscreteVariable> variables = new ArrayList<DiscreteVariable>(fDim + gDim);
        ...
        Function h = createFunction(variables.toArray(new DiscreteVariable[variables
                .size()]));
    ...
    public String toString(int amount) {
        ...
        for (int i = 0; i < domainSize; i++) {
            ...
            for (int j = 0; j < dimension; j++) {
                stringBuffer.append("\"" + _variables[j].getName()+ "(" + _variables[j].getStates().get(states[j]) +")" + "\" ");
            }
    ...
    public void reorderStates(DiscreteVariable variable, int[] stateOrder) {
        ...
        iterator.iterate(new StateReorderingVisitor(variables.indexOf(variable), stateOrder));
    }
    ...
    /**
     * En este caso las celdas se encuentran ordenadas segun Weka y el "nuevo" orden es el que propone Voltric
     */
    @Deprecated
    private double[] reorderCellsNotFinished(List<DiscreteVariable> currentOrder, List<DiscreteVariable> newOrder, double[] cells){

        if(currentOrder.size() != newOrder.size())
            throw new IllegalArgumentException("Both arrays of variables should have the same size");

        for(DiscreteVariable var: currentOrder)
            if(!newOrder.contains(var))
                throw new IllegalArgumentException("Both arrays must contain the same set of variables");

        int cptDimension = currentOrder.size();

        int[] cptIndexMap = new int[cptDimension];// variables map
        int[] cptStates = new int[cptDimension];

        // Sabiendo el nuevo orden, hacemos un map con sus índices de las variables.
        for(int i = 0; i < cptDimension; i++)
            cptIndexMap[i] = newOrder.indexOf(currentOrder.get(i));

        // Una vez que hemos mapeado las variables vamos a ir transformando


        throw new IllegalArgumentException("Not finished");
    }

    /** Given a disordered function, we create a new one and assign its corresponding cells */
    public Function reorderWekaFunction() {

        Function orderedFunction = createFunction(this.getVariables()); // sorts the variables' vector
        int[] cptIndexMap = new int[orderedFunction.getDimension()];// variables map
        int[] cptStates = new int[orderedFunction.getDimension()];

        // Sabiendo el nuevo orden, hacemos un map con sus índices de las variables.
        for(int i = 0; i < orderedFunction.getDimension(); i++)
            cptIndexMap[i] = orderedFunction.getVariables().indexOf(this._variables[i]);

        // Iteramos por las celdas de la CPT
        for(int i = 0; i < this.getDomainSize(); i++){
            // Obtenemos el vector de states asociado a la celda
            this.computeStates(i, cptStates);

            // Mapeamos dicho vector de estados al orden de Voltric
            int[] mappedStates = new int[cptStates.length];
            for(int j = 0; j < cptStates.length; j++)
                mappedStates[cptIndexMap[j]] = cptStates[j];

            // Asignamos el valor en su celda correspondiente
            orderedFunction._cells[orderedFunction.computeIndex(mappedStates)] = this._cells[i];
        }

        return orderedFunction;
    }

    public static Function createWekaFunction(List<DiscreteVariable> variables, List<Double> cells){

        double[] cellsArray = new double[cells.size()];
        for(int i = 0; i < cellsArray.length; i++)
            cellsArray[i] = cells.get(i);

        DiscreteVariable[] variablesArray = variables.toArray(new DiscreteVariable[variables.size()]);

        // builds magnitude array with variables[0] at most significant bit
        int[] magnitudes = new int[variables.size()];

        int magnitude = 1;
        for (int i = variables.size() - 1; i >= 0; i--) {
            magnitudes[i] = magnitude;
            magnitude *= variables.get(i).getCardinality();
        }
        ...
        switch (variables.size()) {
            ...
            case 1:
                f = new Function1D(variablesArray, cellsArray, magnitudes);
                break;

            case 2:
                f = new Function2D(variablesArray, cellsArray, magnitudes);
                break;

            default:
                f = new Function(variablesArray, cellsArray, magnitudes);
                break;
        }
